package lu.uni.dapreco.parser.owl;
...
public class Ontology {
	...
	/**
	 * Helper function that returns the an OWL object property identified by name
	 * and namespace.
	 * 
	 * @param name
	 *            the OWL object property whose name or label is requested
	 * @param ns
	 *            the namespace of the requested object property
	 * @return the first OWL annotation containing a label for the entity
	 */
	public OWLObjectProperty getObjectPropertyByFullLabel(String name, String ns) {
		for (OWLObjectProperty o : ontology.getObjectPropertiesInSignature()) {
			String p = o.getIRI().getNamespace();
			if (p.equals(ns)) {
				if (o.getIRI().getShortForm().equals(name))
					return o;
				for (OWLAnnotation a : EntitySearcher.getAnnotations(o, ontology)) {
					if (a.getProperty().isLabel()) {
						OWLLiteral label = (OWLLiteral) a.getValue();
						if (label.getLiteral().equals(name))
							return o;
	...
	/**
	 * Helper function that returns the an OWL data property identified by name and
	 * namespace.
	 * 
	 * @param name
	 *            the OWL data property whose name or label is requested
	 * @param ns
	 *            the namespace of the requested data property
	 * @return the first OWL annotation containing a label for the entity
	 */
	public OWLDataProperty getDataPropertyByFullLabel(String name, String ns) {
		for (OWLDataProperty d : ontology.getDataPropertiesInSignature()) {
			String p = d.getIRI().getNamespace();
			if (p.equals(ns)) {
				if (d.getIRI().getShortForm().equals(name))
					return d;
				for (OWLAnnotation a : EntitySearcher.getAnnotations(d, ontology)) {
					if (a.getProperty().isLabel()) {
						OWLLiteral label = (OWLLiteral) a.getValue();
						if (label.getLiteral().equals(name))
							return d;
